...
inline bool     operator <  (const timespec& lhs, const timespec& rhs) { return TimerFrame::TimeSpecCmp(lhs, rhs) <  0;                                }
inline bool     operator >  (const timespec& lhs, const timespec& rhs) { return TimerFrame::TimeSpecCmp(lhs, rhs) >  0;                                }
inline bool     operator == (const timespec& lhs, const timespec& rhs) { return TimerFrame::TimeSpecCmp(lhs, rhs) == 0;                                }
inline bool     operator >= (const timespec& lhs, const timespec& rhs) { int cmp = TimerFrame::TimeSpecCmp(lhs, rhs); return (cmp == 0) || (cmp > 0);  }
inline bool     operator <= (const timespec& lhs, const timespec& rhs) { int cmp = TimerFrame::TimeSpecCmp(lhs, rhs); return (cmp == 0) || (cmp < 0);  }

inline timespec operator -  (const timespec& lhs, const timespec& rhs) { return TimerFrame::TimeSpecDiff(lhs, rhs);      }
...
class TickThread : public Thread
{
    public:
        ...
        TimerFrame      TickTimeFrame()     { return _tickTs;    };
